...
testRig : Iso (j [ B² ]) (B²) → ℕ → 𝔹
testRig α n = all (λ { (x , y) → ==ADT { j [ B² ] } x y }) (List→ f (allB² n))
  where i1 = c× =!~ α
        i2 = α ~!= j[c×B²]
