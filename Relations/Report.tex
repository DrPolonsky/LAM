...
\begin{dfn}
  A reduction relation is \emph{confluent} (AKA Church-Rosser (CR)) if all multi-step reductions from a given element can always converge via multi-step reductions to some common reduct.
\end{dfn}
    \begin{figure}
      \centering
      \begin{tikzpicture}[node distance=1.5cm, auto]
...
\begin{dfn}
  ...
  A relation $\to$ is weakly normalizing (WN) if every $a \in A$ is $R$- weakly normalizing.
\end{dfn}
...
\begin{dfn}
  ...
  A relation $\to$ is strongly normalizing (SN) if every
  $a \in A$ is $\to$-strongly normalizing.
\end{dfn}
...
\apcomment{This is not quite right; rather, $\to^=$ is the reflexive-symmetric-transitive closure of $\to$, i.e., it is the \emph{equivalence relation generated by $\to$}, the NFP refers to this latter relation.}

\apcomment{Add a subsection/definition listing the closure operations on relations.}
...
\begin{dfn}
  $\to$ has the \emph{recurrence property} (RP) if every
...
\end{dfn}
    ...
    \begin{figure}[h]
      ...
      \begin{tikzpicture}
        ...
        \end{figure}
\begin{dfn}
  $\to$ has the \emph{weak recurrence property} (RP-) if
