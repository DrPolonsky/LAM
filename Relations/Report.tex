...
\begin{dfn}
  An abstract reduction system (\emph{ARS}) is a structure $\mathcal{A} = (A, \to_\alpha)$ where $A$ is a set and $\to_\alpha$ is a set of binary relations.    
\end{dfn}
A relation $\to$ from the set $\to_\alpha$ is refered to as a reduction (or rewrite) relation. For example, if we specify $\to$ as the relation of being 'less than' ($<$) then $4 \to 2$. \textcolor{red}{Right?}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[node distance=1.5cm, auto]
    ...
    \draw[->>] (right) -- (bottom);
\end{tikzpicture}
\caption{An example of a weakly confluent relation.}
\end{figure}
...
\end{dfn}
    \begin{figure}
      ...
      \begin{tikzpicture}[node distance=1.5cm, auto]
        ...
        \draw[->>] (right) -- (bottom);
      \end{tikzpicture}
    \caption{An example of a confluent relation.}
    \end{figure}
    
\begin{dfn}
  \emph An element $a \in A$ is a {normal form} if there exists no reduction relation to any other element in $A$.
\end{dfn}
    
\begin{dfn}
  $a \in A$ is \emph{weakly normalizing} if $a \tto b$ for some normal form $b \in A$. 
  $\to$ is weakly normalizing (WN) if every $a \in A$ is weakly normalizing.  
\end{dfn}
\sacomment{A lot of these are near word for word from TeReSe, is that okay?}
    
\begin{dfn}
  $a \in A$ is \emph{strongly normalizing} if every reduction sequence starting from $a$ is finite. $\to$ is strongly normalizing (SN) if every 
  $a \in A$ is strongly normalizing. 
\end{dfn}

The above definition of SN is taken from TeReSe. We make use of the alternative definition presented in TeReSe ($\leftarrow$ is accessibly well founded) when working in Agda.  

If $\to$ is a reflexive, transitive and symmetric relation we denote it $\to^=$.

\begin{dfn}
  A reduction relation $\to$ has the \emph{normal form property} (NFP) if 
 $a \to^= b$  $\implies$
 $a \tto b$, for all $a \in A$ and any normal form $b \in A$. 
\end{dfn}

$a, b \in A$ being equivalent is denoted $a \equiv b$. 

\begin{dfn}
  $\to$ has the \emph{unique normal form property} (UN) if 
  $a \to^= b$  $\implies$
  $a \equiv b$, for all $a \in A$ and any normal form $b \in A$. 
\end{dfn}

\begin{dfn}
  $\to$ has the \emph{unique normal form property with respect to reduction} (UN$^ \to$) if 
  $a \twoheadleftarrow  \cdot  \tto b$  $\implies$ $a \equiv b$, for all normal forms $a,b \in A$. 
\end{dfn}

\begin{dfn}
  A \emph{sequence} is a relation from the natural numbers onto a set: $\nat \to A$.
\end{dfn}

\begin{dfn}
  A sequence is \emph{increasing} if every term is greater than (\sacomment{or equal to?}) its previous term.
\end{dfn}

\begin{dfn}
  A sequence is \emph{$\omega$-bounded} if there exists an element $a$ to which all elements of the sequence 
  reduce. 
\end{dfn}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
      ...
      \node (a) [below=of dots, yshift=-.25cm] {\parbox{2cm}{\centering $a$ }};
          ...
          \draw[->>] (dots) -- (a);
      \end{tikzpicture}
      \caption{An example of an $\omega$-bounded sequence.}

    \end{figure}


\begin{dfn}
  $a \in A$ has the \emph{recurrence property} if $a \tto b \implies b \tto a$ for all $b \in A$. 
\end{dfn}
...
\begin{dfn}
  $\to$ has the \emph{recurrence property} (RP) if there is an increasing sequence $f$ in $A$ with an $\omega$-bound and 
  there is an element of the sequence that has the recurrence property. 
\end{dfn}
    ...
    \begin{figure}[h]
      ...
      \begin{tikzpicture}
        ...
        \end{figure}
\begin{dfn}
  $\to$ has the \emph{recurrence property -} (RP-)  if there is an increasing sequence $f$ in $A$ with an $\omega$-bound $a$ and 
...
\begin{dfn}
  $\to$ is \emph{finitely branching} (FB) if, for all $a \in A$, the set of one step reducts of $a$ is finite. 
\end{dfn}
