...
\title{Formalizing Abstract Rewriting}
% \title{Abstract Rewriting from a Constructive Viewpoint}
% \title{A Constructive View on Abstact Rewriting}
\author{Sam Arkle}
\date{\today}
...
\maketitle
\begin{abstract}
We formalize in the Agda proof assistant the main results about Abstract Reduction Systems (ARS) given in the standard reference [Terese].
% Our contribution can be viewed as a development of abstract rewriting using constructive logic.
% , and is part of a larger project aiming to develop a library of programming language theory in Agda. This application leads us to give special focus to concepts related to termination and confluence.
We define a taxonomy of concepts related to termination and confluence and investigate their relationships between each other and their classical counterparts.
We identify, and eliminate where possible, the use of classical logic in the proofs of standard ARS results.
% For example, the study of strong normalization leads us to multiple inequivalent
% formalizations of the notion of a well-founded relations, with subtle distinctions.
Our analysis leads to refinements and mild generalizations of classical termination and confluence criteria.
\end{abstract}
...
\begin{dfn}
  ...
  $a \in A$ is $R$-strongly recurrent.
\end{dfn}
\sacomment{We have the two contender definitions for SR above.}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \[\lnot\lnot \big(\exists n \in \nat. s n \equiv a\big)
          \to \big(\exists n \in \nat. s n \equiv a\big)\]
  \end{itemize}
  \item When an element is not SN, it transitions to one which is not SN either.
  (Use in the proof of Theorem 1.2.3)
\end{itemize}
...
\begin{center}
  ...
  \begin{tikzpicture}[auto,
    ...
    % A box style
    \tikzstyle{boxnode} = [draw, rectangle, minimum width=3cm, minimum height=1cm, text centered]

    % Place the nodes vertically (top to bottom)
    \node (Confluent) [boxnode] at (0,0) {Confluent};
    \node (RP)        [boxnode, below=2cm of Confluent] {Recurrence Property};
    \node (WN)        [boxnode, below=2cm of RP] {Weak Normal Form Property};
    \node (UN)        [boxnode, below=2cm of WN] {Unique Normal Form Property};
...
\begin{tikzpicture}[node distance=2cm, auto,
  % Define a thicker arrow with larger arrowhead and some shortening
  arrowstyle/.style = {->, line width=1pt, >={Latex[length=3mm, width=2mm]}, shorten >=2pt}
]
% Define a box style
\tikzstyle{boxnode} = [draw, rectangle, minimum width=3cm, minimum height=1cm, text centered]
...
% Define the nodes
\node (SN) [boxnode] at (0,0) {Strongly Normal};
\node (WN) [boxnode, below left=2cm and 2cm of SN] {Weakly Normal};
\node (SR) [boxnode, below right=2cm and 2cm of SN] {Strongly Recurrent};
\node (WR) [boxnode, below of=SN, yshift=-4cm] {Weakly Recurrent};
...
\begin{table}[h!]
  ...
  \hline
  \textbf{CR} & CR, CE-9  & CR, CE-6  & CR , CE-9  & CR, SN  & CR , SN \\
  \hline
  \textbf{RP} & CR, CE-9  & CR, CE-6  & \red{CR} , CE-9 & CR, SN   & \red{CR} , SN \\
  \hline
  \textbf{WNFP} & CE-10, CE-9  & CR, CE-6  & CE-11 , CE-9 & CR, SN   & \red{CR} , SN \\
  \hline
...
\begin{table}[h!]
  ...
  \textbf{RP} & CR, CE-9  & CR, {CE-6}  & \red{CR} , {CE-9} & CR, SN   & \red{CR} , SN \\
  \hline
  \textbf{WNFP} & CE-10.1, CE-9  & CR, {CE-6}  & {CE-11} , {CE-9} & CR, SN  & \red{CR} , SN \\
  \hline
...
\begin{table}[h!]
  ...
  \textbf{RP} & CR, {CE-9}  & CR, {CE-6}  & \red{CR} , {CE-9} & CR, SN   & \red{CR} , SN \\
  \hline
  \textbf{WNFP} & {CE-11}, {CE-9}  & CR, {CE-6}  & CE-11 , {CE-9} & CR, SN   & \red{CR} , SN \\
  \hline
  \textbf{UN} & {CE-3}, {CE-12} and CE-9  & CR, {CE-6}  & CE-3, {CE-9}  & CR, SN  & \red{CR} , SN \\
  \hline
  & \textbf{WR} & \textbf{WN} & \textbf{SR} & \textbf{WN}$\boldsymbol{\land}$\textbf{SR} & \textbf{SN} \\
  \hline
  \end{tabular}
...
\begin{table}[h!]
  ...
  \textbf{CR} & CR, {CE-9}  & CR, {CE-6}  & CR , {CE-9}  & CR, SN  & CR , SN \\
  \hline
  \textbf{RP} & CR, {CE-9}  & CR, {CE-6}  & {CR} , {CE-9} & CR, SN   & CR , SN \\
  \hline
  \textbf{WNFP} & CE-1.2, {CE-9}  & CR, {CE-6}  & {CR} , {CE-9} & CR, SN   & CR , SN \\
  \hline
  \textbf{UN} & CE-1.1, {CE-9}  & CR, {CE-6}  & {CR}, {CE-9}  & CR, SN  & CR , SN \\
  \hline
  & \textbf{WR} & \textbf{WN} & \textbf{SR} & \textbf{WN}$\boldsymbol{\land}$\textbf{SR} & \textbf{SN} \\
  \hline
  \end{tabular}
...
Counterexample to:
\begin{itemize}
  \item (Global) WCR, UN, and WR $\implies$ WNFP
\end{itemize}

\subsection*{Counterexample 1.2}
\begin{center}
...
Counterexample to:
\begin{itemize}
  \item (Global) WCR, WNFP, and WR $\implies$ RP
\end{itemize}
...
Counterexample to:
\begin{itemize}
  \item (Local) WCR, WR, and UN $\implies$ WN
...
\subsection*{Counterexample 10.1}
\begin{center}
  \begin{tikzpicture}
      % Nodes
      \node (a) {a};
...
Counterexample to:
\begin{itemize}
  \item (Local) WCR, WR, and NP $\implies$ RP
\end{itemize}
...
Counterexample to:
\begin{itemize}
    \item (gloabl) WCR, WR, and UN $implies$ WN or SR or SN
\end{itemize}
