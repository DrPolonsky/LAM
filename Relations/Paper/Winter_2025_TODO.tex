...
\begin{todolist}
  ...
  \item can we show that  wfminDNE implies RisDec. We've shown that RisWFmin implies RisDec (in WFcounters). We've started the DNE proof in Wfimplications (commented out). 
  \item [\done] We successfully proved that RisWFmin implies RisDec and RisMinDec!
  \item We're trying to show that wfminDNE implies WEM (in WFcounters) (this is where we ended the session 9th December). Try using the commented out code to solve the goal on line 130. 
  \item Ideal goal for next meeting is to formalize trs.
\end{todolist}  
