...
% As the figure illustrates, the concept of well-foundness is very rich from the vantage point of constructive logic.
% Our analysis resulted in the following principles, 
%
% The additional classical hypotheses needed to recover some of the implications on the diagram 
%
...
%
As can be seen from the figure, taking the $\lnot\lnot$-closures of 
...
the (weak version of) accessible one without an additional hypothesis $\accCor$, 
...
As the Figure \ref{tab:cprop} illustrates, the network of relationships between the
constructive notions of well-foundedness is quite rich. 
The main observations can now be read off from the figure.
\begin{enumerate}
  ...
  (assuming decidability of equality on $A$).
  ...
  implies WFacc- directly.  It should be noted that, $\FB$ together with (weak) decidability of minimality
  implies $\accCor$.  \apcomment{Fix this: This is proved in \texttt{FinitelyBranching.agda}.}.
  ...
  \item Assuming $\accCor$, $\isWFacc$ ($\isWFaccm$) can also be derived from a weaker condition, $\WFcor$ ($\WFcorm$).
  ...
  \apcomment{True?  It is implied by $\isWFseqm$, but does not appear to imply anything on its own.}
