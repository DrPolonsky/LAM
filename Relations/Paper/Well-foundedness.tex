...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item  $R$ is \emph{minimally well-founded} ($\WFmin$) if every nonempty subset has a $P$-minimal element:
      ...
      \]
    \item  $R$ is \emph{minimally well-founded for $\lnot \lnot$-closed predicates} ($\WFminDNE$) if every nonempty $\lnot \lnot$-closed subset contains a $P$-minimal element:
      \[
...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item $R$ is \emph{weakly sequentially well-founded} ($\WFseqm$) if no sequence is $R$-decreasing:
      ...
      \]
    \item  $R$ is \emph{weakly minimally well-founded} ($\WFminm$) if every nonempty subset contains a $\lnot \lnot P$-minimal element:
      \[
        \forall P \subseteq A.\; P \neq \varnothing \Rightarrow \lnot \lnot \;
        (\exists x \in P.\; \text{$x$ is $P$-minimal}).
      \]
    \item  $R$ is \emph{weakly minimally well-founded under double negation elimination} ($\WFminDNEm$) if every nonempty $\lnot \lnot$-closed subset $\lnot \lnot$ contains a $P$-minimal element:
      \[
