...
\draw[arrow , bend left = 20] (WFmin) to node [below, text = red] {$\Rne$} (WFacc);
...
\midrule
$\EM$     & All predicates are decidable 
          &  $\mathtt{\forall (P : \mathcal{P} A) \to P \lor \lnot P}$ \\
$\rdec$   & $R$ is decidable
...
\begin{enumerate}
  ...
  \item The implication from $\WFmin$ to $\WFacc$ requires the underlying relation to be non-empty. \sacomment{Do we want to say anything more here?}
  ...
  \item If the relation is finitely branching ($\FB$) or accessibility is $\lnot\lnot$-closed ($\accDNE$) on the relation, then $\WFminDNEm$
...
\[
  \tag{$\isMinDec$} \forall x. \left(\exists y. x \rstep y \right) \lor \left(\forall y. x \,\,\not \!\!\!\!\rstep y\right)
\]
