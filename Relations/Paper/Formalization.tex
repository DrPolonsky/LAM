...
As mentioned, the $\Inc$ property imposes a computationally demanding requirement. However, $\Inc$ is used only in Theorem 1.2.3 
and the only crucial aspect of $\Inc$ needed for the proof is that no $R$-increasing 
sequence can have an upper bound.
We establish that the properties $\RP$ and $\RPm$ were sufficient for replacing $\Inc$ in the theorem (see Subsection \ref{subsec:theorems}), 
which led us to examine the condition in $\RP$ more closely. This condition provides a generalization of the notion of a normal form, 
...
The first and third of these proofs make use of classical assumptions. The second proof is amenable to a 
...
to construct a witness for an element that either does or does not have the property $\SN$. Such a witness was necessary for our proof. \sacomment{Link to decidable in later section?}
