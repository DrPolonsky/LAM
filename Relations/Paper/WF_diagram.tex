...
% Row 3: WFseq
\node[box, below=of WFind] (WFseq) {WFseq};
...
\draw[arrow] (WFind) to node[right, text = red] {Rdec} (WFseq);
\draw[arrow, bend left] (WFindM) to (WFseqM);
\draw[arrow, bend left] (WFseqM) to node[left, text=red] {accCor} (WFindM);  
\draw[arrow, bend left = 45] (WFindM) to (WFminM);  
...
\draw[arrow, bend left=60] (WFminDNE) to node[left, text = red] {accDNE} (WFacc);
