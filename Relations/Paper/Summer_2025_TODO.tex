...
\begin{todolist}
  \item (Secondary) Find consistency with NP and UN with respect to reduction and conversion. Should the default be by reduction for both. Should we be explicit with $\to$ and = in each case. 
  \item Carry on modifying the names of wfXXX- to wfXXXnotnot with prefix underscore in agda and in latex.
  \item Make a decision regarding WFcor. We use it to imply WFacc using AccCor. But otherwise it is sort of orphaned off. Does it help contribute to the overall story?
\end{todolist}
