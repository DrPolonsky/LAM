...
\begin{itemize}
  ...
  \begin{todolist}
    \item [\done] (Review 1) Elaborate on our motivations in the introduction (reviewer wasn't sure how relevant this is to ARS theory itself/ sees it as too minor a contribution.)
    ...
    \item [\done] (Review 2) Unconvinced by the motivations for the project, believes the same can be achieved in a classical framework. So maybe we should explain our motivations more clearly.
    ...
    \item [\done] (Review 3) Check out and include the following related work:
    \begin{itemize}
...
\begin{itemize}
  \item[\done] Perhaps we should not create variations and multiple implications of WFind,
...
\section*{July TODO}
\begin{todolist}
\item [\done] Carry on refactoring Wellfounded.agda once fixed, breaking it up if necessary.
\item [\done] To finish the paper: We should do a final walk through and make sure we haven't introduced errors with our edits. In particular, do all the counterexamples line up. Do all the references to different sections and pieces of code in Agda line up. 
  \item [\done] Do we remove all our local counterexamples in favour of global ones?
  \item [\done] Rework well-foundedness section 
  \item[\done]  Rework conclusion 
  \item [\done]  Rework introduction 
  \item  [\done] Do we introduce a new section for discussing Agda? Talking about why we think there is an advantage to doing this work in Agda vs (say) Isabelle 

\end{todolist}

\section*{Research items}
\begin{todolist}
\item[\done] (Research item) Go deeper in the related works and see which are most relevant. Make a list of works and give summaries to be used in future papers. * Look into the literature and related works. There should be an Agda formulation of rewrite systems (check agda standard library stuff on relations). 
...
\begin{todolist}
\item[\done] Write about the standard library formalization in our paper and talk about how ours differs. 
\item[X]  Create an equivalences file showing that our formalization agrees with the ASL (specifically definitions such as confluet, wcr, nf, leading up to showing our formalizations of Newman's lemma agree).
...
\begin{todolist}
  ...
  \item [\done] Carry on modifying the names of wfXXX- to wfXXXnotnot with prefix underscore in agda and in latex. \sacomment{Done with the exception of WFseq-. This needs a bit more thought. Similarly for the relation between WFcor and WFminCor.}
  ...
  \item [\done] Make a cheat sheet for the next meeting regarding to talk about coreductive. How it relates on the weak side and on the strong side. SEQ seq+- , how to present these. Try and clarify what the issues are here and what exactly are the differences between them in terms of definition and implication. Could one be removed and the overall story stay the same? \sacomment{Regarding seq+ (and seq+-) we currently only show that seq+ implies seq and seq+- implies seq- so could be removed without changing any of our other implictions.} 
\end{todolist}

\section*{September 15th todo}
\begin{todolist}
\item[\done] (Minor) We want to have a sentence in our WF section making clear that we can consider local properties as well and maybe make an example from accessible to sequential. 
  \item[\done] Sentence explaining that wfacc is equivalent to wfind and therefore isn't included in the diagram (wfind).
\end{todolist}

\section*{September 26th TODO}
\begin{todolist}     
\item[\done] Focus for next meeting on writing. 
...
\begin{todolist}     
\item[\done] Want to show that FB and weak decidability gives us accCor (think we may have shown this previously)
  \item[\done] Add 'is' to wfcor macros.
  \item[\done] make a decision on wfseq not not name. 
  \item[\done] Keep on cleaning the WF section in paper. 
\end{todolist}
...
\begin{todolist}     
  ...
  \item [\done] Do a final read through of the information below the figure. Have we captured all the most important information / Are we repeating ourselves?
  \item [\done] Do we want to keep anything in the old/delete section of WF?
  \item [\done] Do we want to add anything about counter examples or positive examples? 
...
\item [\done] Make sure we have a proof of the implication from SN to WN given FB and decidability of a relation (to back up our claim in our introduction).
...
\item [\done, did SN to WN instead] update wfcounters to show that asserting wfmin implies the excluded middle (and wfmindne the weak excluded middle).
...
\begin{todolist}
  ...
  \item[\done] Make a main file at this point to conform to this conference's style. Possibly a sub-folder for each conference.
  \item [\done] discussion env has qed boxes which shouldn't be there.
  \item[\done] from wellfounded extra move the implication of fb -> isdec -> isMinDec. move to implications. 
  ...
  \item [\done] Make consistent how we display definitions. 
  \item [\done] Work on the conclusion
  \item [\done] Need to fix how we put urls in for bibliography. Currently not quite working. \sacomment{Now all urls, if happy with how it looks we can tick it off. }
  \item [\done] Check the PLDI page for submission rules/requirements. In particular the bibliography style 
  \item [\done] Run through and compile everything, make sure we didn't introduce any bugs
  \item [\done] Check the layout of figures through the paper. Counterexamples for example is currently `off'.
  \item [\done] talk about why we use agda in 1.2 Agda is the perfect system for formalizing programming language theory.  
  \item Submit 
  \item [\done] Add a paragraph or two giving a short overview/summary of Agda.
  ...
  \item Consider adding some more motivations 
  \item Look at the seq-minus vs. seq-not-not issue 
  \item Final read through (wed?)
  \item [\done] make an external bibliography file rather than inlining in main file. Make sure bibliography is up to standard (e.g. emph on book title).
  \item Submission deadline is Thursday 13th 
\end{todolist}

\section*{Post pldi todos}
\begin{todolist}
...
notes to self:
- Tell Dr. P to check out the trs pvs paper. 
