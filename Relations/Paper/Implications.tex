...
\begin{definition}\label{def:mf} Let $a \in A$. \hfill
    \begin{description}
        \item[\bq \ule{$a \in \MF_R$}] \emph{$a$ is a minimal form} if $a \mstep b \implies b \mstep a$ for any $b \in A$.
        \item[\bq \ule{$a \in \WM_R$}] \emph{$a$ is weakly minimalizing} if $a \mstep b$ for some $b \in \MF_R$.
        \item[\bq \ule{$a \in \SM_R$}]  \emph{$a$ is strongly minimalizing} if either $a \in MF_R$, or every element one $R$ step from $a$ is strongly minimalizing. (This
        definition is to be understood inductively).
        \item[\bq \ule{$a \in \SMseq_R$}] \emph{$a$ is sequentially strongly minimalizing} if for every $R$-increasing sequence $s$ starting at $a$, there exists
        an $i \in \nat$ such that $s (i) \in \MF$.
        \item[\bq \ule{$a \in \MP_R$}] \emph{$a$ has the minimal form property} if $c \bmstep a \mstep b \implies c \mstep b$ for any $b \in \MF$.
    \end{description}
\end{definition}
...
A counterexample is furnished in the file \texttt{WFCounters.agda}.
...
This is formalized in file \apcomment{Need to fill this in.}

Since these conditions are in fact satisfied by all of the standard
term rewrite systems and lambda calculi, we see that for such ARSs the implication
