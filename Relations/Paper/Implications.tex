...
As discussed in Subsection \ref{subsec:def} we put forward $\RP$ as an alternative property for $\Inc$. It is through examining $\RP$ that 
...
We show that $\SM \implies \SMseq$, however $\SMseq$ is only classically equivalent to $\SM$. The 
...
The isMinDec property \sacomment{refer to list of classical properties here} states that all elements either have a relation to some other element, or 
...
and so trivially $\WN$. \greyout{If the element does relate to some other element $y \in A$, then we recursively apply our claim to 
...
\begin{remark} The following are the key takeaways from the implication tables. \hfill
    \begin{enumerate}
        ...
        \item $\gWN \land \gUNto \implies \gCR$ doesn't require any classical assumptions
         or $\gWCR$ for $\gCR$, unlike $\gSN \land \gUNto$. 
